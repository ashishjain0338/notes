...
1. Create a Post [Write Operation]
2. Follow/unfollow [Write Operation]
3. Like/Comment [Write Operation]
4. Instagram Feed [Read Operation]

#### A.1: Read-Throughput
Assume every user scrolling the feed sees an average of <span style={{color: "red"}}>100 post per day</span>.
```math
Total Read operations per day = DAU * 100 = 500 M * 100 = 50 B read-request per day.
```

#### A.2 Write-Throughput
Assume creating a post constitute a major chunk of write operations, neglecting others. And also <span style={{color: "red"}}>10% of users</span> posts a post everyday.
